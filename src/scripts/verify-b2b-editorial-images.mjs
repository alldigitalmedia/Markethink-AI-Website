...
].map(([placement, title, route, desktop, mobile, thumbnail, desktopWidth, desktopHeight, mobileWidth, mobileHeight, thumbnailWidth, thumbnailHeight]) => ({
  placement,
  title,
  route,
...
assert.equal(new Set(b2bEditorialImages.flatMap(({ desktop, mobile, thumbnail }) => [desktop, mobile, thumbnail])).size, 30, "Every derivative URL must be unique");
...
for (const token of ["imageThumbnail", "b2bEditorialImages"]) assert.ok(sources.data.includes(token), `blogPosts.ts must expose ${token}`);
